 ...
 *
 * Client component pga clipboard API + Window-objektet behövs.
 */
...
export default function BlogShareButtons({ url, title }: BlogShareButtonsProps) {
  ...
  const onCopy = async () => {
    try {
      ...
      // Clipboard kan blockeras (iframe etc) — fallback: prompt
      window.prompt(t('shareCopy'), url)
    }
  }

  const buttonClasses =
    'inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-white/80 bg-et-surface border border-et-border rounded-lg hover:border-eteya-yellow/50 hover:text-eteya-yellow transition-colors'
